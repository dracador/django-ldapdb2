    ...
    def execute_sql(self, returning_fields=None):  # noqa: ARG002
        ...
        with db.wrap_database_errors:
            try:
                ...
                raise NotSupportedError(f'Entry {dn!r} vanished while updating')
        ...
        for field, _model, raw_val in self.query.values:
            ...
            if not new_vals:
                ...
                if getattr(field, "update_strategy", UpdateStrategy.REPLACE
                           ) == UpdateStrategy.ADD_DELETE and field.multi_valued_field:
                    mod.delete(attr)
                    mod.add(attr, new_vals)
                else:
